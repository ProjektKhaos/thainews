...
use ThaiNews\Metadata; use ThaiNews\Security\Csrf;
...
if($_SERVER['REQUEST_METHOD']==='POST'){
 if(!Csrf::validate($_POST['csrf']??null)){http_response_code(400);} else {
  $lang=in_array($_POST['language']??'', $supported,true)?(string)$_POST['language']:'en'; $slugs=array_values($_POST['source_slug']??[]);$visible=$_POST['visible']??[];$sources=[];
  ...
  try{$repo->save($visitor,$lang,$sources); header('Location: '.url('settings.php?lang='.$lang.'&saved=1')); exit;}catch(Throwable){http_response_code(422);}
 }
}
